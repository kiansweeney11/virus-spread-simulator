...
healthy = total_population - infections_initial
...
def initialise():
    ...
    time = 0
    agents = []
    for i in range(total_population - infections_initial):
        ag = agent()
        ag.state = 'healthy'  if random() <= probability_infection else 'immune'
        if ag.state == 'immune':
            immune += 1
            healthy -= 1
            # time agent is immune from virus after having it
            ag.immunity_time = 30
        ag.infected_time = -1
    ...
    for i in range(infections_initial):
        ag = agent()
        ag.state = 'infected'
...
def update_one_agent():
    ...
    # current agent is infected -> all neighbours become infected unless immune or vaccinated
...
def update():
    ...
    # change state of agents who have been infected for 14 days to either immune/recovered
...
def set_vaccination_rate(val = vaccinate_rate):
## adjust vaccination rate. between 0 and 1.
    global vaccinate_rate
    vaccinate_rate = float(val)
    return val

def set_starting_population(val = total_population):
## change initial population. default as 1000.
    global total_population
    total_population = int(val)
    return val

def set_starting_infections(val = infections_initial):
## change number of infections starting off. must be less than total population. defaults as 10.
    global infections_initial
    infections_initial = int(val)
    return val

def set_probability_infection(val = probability_infection):
## set probability of initial infection. between 0 and 1. best kept closer to 1.
    global probability_infection
    probability_infection = float(val)
    return val

def set_probability_immunity(val = probability_immunity):
## change probability of immunity. between 0 and 1. keep around 0.5 region preferably.
    global probability_immunity
    probability_immunity = float(val)
    return val

def set_probability_vaccination_infection(val = probability_vacc_infection):
## set probability of getting virus if vaccinated.  between 0 and 1. closer to 1 means very effective vaccine, very little
##  infections of vaccinated agents
    global probability_vacc_infection
    probability_vacc_infection = float(val)
    return val

def set_reinfection_chance(val = probability_immunity_reinfection):
## probability of reinfection. again between 0 and 1
    global probability_immunity_reinfection
    probability_immunity_reinfection = float(val)
    return val

pycxsimulator.GUI(parameterSetters=[set_starting_population, set_starting_infections, set_vaccination_rate, set_probability_vaccination_infection,
set_probability_infection, set_reinfection_chance, set_probability_immunity]).start(func=[initialise, observe, update])
...
##### Uncomment to track for analysis

#pd.DataFrame.from_records(deaths_lst, columns=['time', 'deaths']).to_csv(os.path.join('./data-analysis/low-vaccine/', 'deaths.csv'), index=False)
#pd.DataFrame.from_records(infections_lst, columns=['time', 'infections']).to_csv(os.path.join('./data-analysis/low-vaccine/', 'infections.csv'), index=False)
#pd.DataFrame.from_records(vaccine_lst, columns=['time', 'vaccinated']).to_csv(os.path.join('./data-analysis/low-vaccine/', 'vaccine.csv'), index=False)
#pd.DataFrame.from_records(reinfection_lst, columns=['time', 'reinfections']).to_csv(os.path.join('./data-analysis/low-vaccine/', 'reinfections.csv'), index=False)